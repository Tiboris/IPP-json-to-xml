...
#JSN:xdudla00
    ...
    function err($errcode) {   // prints only that error occured and error code
        fwrite(STDERR, "Program error, exit code '" . $errcode . "', type '--help' for more info.\n"); // stderr
        die($errcode);
    }
    /*
    ** function parse_args returns or calls help when '--help' is second argument
    ...
    **/
    function parse_args($args, $count)
    {
        ...
        if ( ( $count == 2 ) && ( $args[1] === "--help" ) )  // if only param help is set
            help();
        for ( $i = 1; $i <= $count-1; $i++ ) {
            if( ereg($long_opt_rex_val, $args[$i], $value) || ereg($shrt_opt_rex_val, $args[$i], $value) ) {   // if regex matches options with value, [1]is option [2]is value
                if ( ! isset($parsing[$value[1]]) && ( $value[2] != null ) )  // when script option is not already set
                    $parsing[$value[1]] = $value[2];
                else  // when option is set causes error
                    return false;
            }
            elseif ( ereg($shrt_opt_rex, $args[$i], $value) || ereg($long_opt_rex, $args[$i], $value) ) { // when regex matches options without value, [1]is option [2]is null
                if ( ! isset($parsing[$value[1]]) )
                    $parsing[$value[1]] = true;
                else   // when option is set causes error
                    return false;
            }
            else // when other than specified option is set
                return false;
        }
        return $parsing;
    }
    ...
    **/
    function check_args($args) {
        if ( $args === false )   // when parsing failed
            return false;
        if ( ! isset( $args['input']) )
            $args['input'] = 'php://stdin';
        if (! isset( $args['output']) )
            $args['output'] = 'php://stdout';
        if ( isset( $args['t'] ) && isset( $args['index-items'] ) )  // when -t is set --index-items can not be set it is the same option
            return false;
        if ( isset( $args['start'] ) ) {
            if ( ! isset( $args['t'] ) && ! isset( $args['index-items'] ) ) // start has to be combined with indexing enabled
                return false;
        ...
        if ( isset($args['array-size']) && $args['a'] )   // when -a is set --array-size can not be set it is the same option
            return false;
        return $args;
    }
    ...
    function write($json_input, $args){
        ...
        if ( ! isset($args['n']) ) // header
            $writer->startDocument('1.0','UTF-8');
        if ( isset($args['r']) ) {   // placing result into "root-element" not valid name causes error 50
    ...
    function write_value($writer, $value, $args) {
        ...
        elseif ( ( is_int($value) || is_float($value) ) ) {
            ...
            if (isset($args['i']))    // when option -i is set
                $writer->text($value);
            else
        ...
        elseif ( ( $value === true || $value === false || $value === null ) ) {   // handling litterals true false and null
            if (isset($args['l'])) {   // when -l option is set for replacing literal with <literal/> tag
                if ( $value === true )
        ...
        elseif (isset($args['c']))   // translation of problematic characters
            $writer->text($value);
        else
